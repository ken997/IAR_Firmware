...
void HAL_GPIO_Init(GPIO_TypeDef  *GPIOx, GPIO_InitTypeDef *GPIO_Init)
{
  ...
  for(position = 0; position < ((uint32_t)16U); position++)
  {
    ...
    if(iocurrent == ioposition)
    {
      ...
      temp = GPIOx->MODER;
      temp &= ~((0x3UL << (0U)) << (position * 2));
      temp |= ((GPIO_Init->Mode & (0x3UL << 0U)) << (position * 2));
      GPIOx->MODER = temp;
...
void HAL_GPIO_TogglePin(GPIO_TypeDef* GPIOx, uint16_t GPIO_Pin)
{
  ...
  GPIOx->BSRR = ((odr & GPIO_Pin) << ((uint32_t)16U)) | (~odr & GPIO_Pin);
}
