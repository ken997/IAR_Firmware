...
__noreturn static inline void __NVIC_SystemReset(void)
{
  __iar_builtin_DSB();                                                          
...
void init_os(){
  ...
  highPrioTaskHandle = osThreadNew(highPrioTask, 0, &highPrio_attributes);

   
  AC_CalculationTHandle = osThreadNew(ACCalculationTask, 0, &AC_CalculationT_attributes);

   
  mediumPrio_TaskHandle = osThreadNew(mediumPrioTask, 0, &mediumPrio_attributes);
}
